  ...
  <div class="yesterdayMain">
    ...
    <div class="statistics_cell">
      <div class="customer">
        ...
        <div class="sum-box">
          <div class="sum-item">
            <div class="sum-number">{{userInfo.newCustomerSum || 0}}</div>
            <div class="sum-item-text">昨日新增</div>
          </div>
          <div class="sum-item">
            <div class="sum-number">{{userInfo.lostCustomerSum || 0}}</div>
            <div class="sum-item-text">昨日流失</div>
          </div>
          <div class="sum-item">
            <div class="sum-number">{{userInfo.newGroupCustomerCount || 0}}</div>
...
export default {
  ...
  data() {
    return {
      ...
      myGroupRank: {},
      list: [], // 排行榜数据
      refreshing: false, // 下拉刷新控件 是否处于刷新中
  ...
  methods: {
    ...
    getList() {
      ...
      Http.post(`scrm/comm/rest/daily-summary/page-add-${that.status ? 'group' : 'customer'}-rank-month`, params).then((res) => {
